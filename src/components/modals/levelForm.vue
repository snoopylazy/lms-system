<template>
    <Dialog :visible="open" @update:visible="val => open = val" modal :style="{ width: '95vw', maxWidth: '36rem' }" class="p-2 scrollbar-gold"
        ...
        <template #header>
            <div class="flex items-center space-x-2 text-primary-text border-b-2 border-primary-border w-full p-1">
                ...
                <span class="font-semibold">
                    {{ isEditDoc ? 'Edit Level' : 'Create New Level' }}
                </span>
            </div>
        </template>
        ...
        <form @submit.prevent="handleSubmit" class="space-y-2">
            ...
            <div class="field p-3 rounded-md border border-gray-200 shadow-sm col-span-1">
                ...
                <InputText id="name" v-model="name" :invalid="isExistName" placeholder="Enter name"
                ...
                <div v-if="isExistName" class="mt-2">
                    <Message severity="error">Name already exists!</Message>
                </div>
            </div>
            ...
            <div class="field p-3 rounded-md border border-gray-200 shadow-sm">
                ...
                <InputText id="description" v-model="description" placeholder="Enter description"
                    class="w-full p-3 border border-primary-border rounded-md text-base font-medium focus:border-primary-focus focus:ring-2 focus:ring-primary-light transition-all duration-200" />
            </div>
...
watch(name, async (newV) => {
    if (name.value == originalName.value && name.value !== null) {
        ...
        if (newV) {
            ...
            const dataExist = await fetchAllDoc('Level', {
                dynamicConditions: [
                    {
                        field: "name",
                        operator: "==",
...
const handleSubmit = async () => {
    try {
        ...
        const { add, update } = useDocument('Level');

...
export default {
  ...
  setup(props, { emit }) {
    ...
    async function fetchData() {
      try {
        await handleSetCollectionName(); // run this function to set collection name first
    ...
    async function handleSetCollectionName() {
      ...
      switch (route.path) {
        ...
        case "/users":
          collectionName = "User";
          searchFieldsArray.value = "username,displayName";
          
          // Get userId with fallback logic
          const { getUserId } = await import('@/composables/getUserId');
          const userId = await getUserId();
          
          // Override with user-specific conditions (no branch filter)
          dynamicCondition = [
            {
              field: "_id",
              operator: "!=",
        ...
        default:
          // Try to determine collection name from route name
          if (route.name) {
            const routeName = route.name.toLowerCase();
            // Map route names to collection names
            if (routeName === 'level') {
              collectionName = "Level";
              searchFieldsArray.value = "name";
          ...
          if (!collectionName) {
            console.warn(`Collection name not found for route: ${route.path} (name: ${route.name})`);
          }
          break;
      }
    }

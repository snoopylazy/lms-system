<template>
    <div class="text-primary-text animate-fade-up">
        <h2 class="text-primary-dark text-lg font-bold mb-2">Teacher</h2>
        ...
        <div
            ...
            <div class="flex items-center justify-end">
                <!-- <Button v-if="hasPermission('teacher', 'add') || isSuperAdmin"  -->
                <Button 
                ...
                <table class="min-w-full bg-primary-bg text-center" :class="{ 'opacity-50': isLoading }">
                    ...
                    <thead class="bg-primary-light text-primary-dark text-xs uppercase whitespace-nowrap">
                        <tr>
                            ...
                            <th class="px-3 sm:px-4 py-3 border border-primary-border">Phone</th>
                    ...
                    <!-- Table Body -->
                    <tbody class="text-primary-text text-xs" v-if="teacherData.length > 0">
                        <tr class="hover:bg-primary-hover whitespace-nowrap" v-for="(item, index) in teacherData"
                            ...
                            <td class="px-3 sm:px-4 py-3 border border-primary-border">{{ item.email || 'N/A' }}</td>
                            <td class="px-3 sm:px-4 py-3 border border-primary-border">{{ item.phone || 'N/A' }}</td>
                            ...
                            <td class="px-3 sm:px-4 py-3 border border-primary-border">
                                <div class="flex justify-center gap-3">
                                    <!-- <i v-if="hasPermission('teacher', 'edit') || isSuperAdmin"  -->
                                    <i 
                                    ...
                                    <!-- <i v-if="hasPermission('teacher', 'delete') || isSuperAdmin"  -->
                                    <i 
                                    ...
                                    <!-- <span v-if="!hasPermission('teacher', 'edit') && !hasPermission('teacher', 'delete') && !isSuperAdmin" 
                    ...
                    <tbody v-else>
                        <tr class=" [&>*]:p-2">
                            <td colspan="8" class="font-semibold text-center text-lg">
                                No Data!
                            </td>
                        </tr>
                    </tbody>
...
import TeacherForm from '@/components/modals/teacherForm.vue';
...
const handlePopOpenform = (status, item) => {
    if (status == 'add') {
        openForm.value = true;
        currentComponent.value = TeacherForm;
    } else {
...
const handleConfirmDetele = (item) => {
    docData.value = item;
    elementCategory.value = "Teacher";
    elementName.value = item.name;
    openForm.value = true;
    currentComponent.value = ConfirmationDelete;

    if (teacherData.value.length === 1 && currentPageIsLastRecord.value > 1) {

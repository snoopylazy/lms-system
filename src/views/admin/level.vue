<template>
    <div class="text-primary-text animate-fade-up">
        <h2 class="text-primary-dark text-lg font-bold mb-2">Level</h2>
        ...
        <div
            ...
            <div class="flex items-center justify-end">
                <!-- <Button v-if="hasPermission('level', 'add') || isSuperAdmin"  -->
                <Button 
        ...
        <div class="mt-10">
            ...
            <div class="overflow-x-auto rounded-lg shadow-md border border-primary-border">
                ...
                <table class="min-w-full bg-primary-bg text-center" :class="{ 'opacity-50': isLoading }">
                    ...
                    <!-- Table Body -->
                    <tbody class="text-primary-text text-xs" v-if="levelData.length > 0">
                        <tr class="hover:bg-primary-hover whitespace-nowrap" v-for="(item, index) in levelData"
                            :key="index" :class="index % 2 === 0 ? '' : 'bg-primary-30'">
                            <td class="px-3 sm:px-4 py-3 border border-primary-border">{{ index + 1 }}</td>
                            <td class="px-3 sm:px-4 py-3 border border-primary-border">{{ item.name }}</td>
                            ...
                            <td class="px-3 sm:px-4 py-3 border border-primary-border">
                                <div class="flex justify-center gap-3">
                                    <!-- <i v-if="hasPermission('level', 'edit') || isSuperAdmin"  -->
                                    <i 
                                    ...
                                    <!-- <i v-if="hasPermission('level', 'delete') || isSuperAdmin"  -->
                                    <i 
                                    ...
                                    <!-- <span v-if="!hasPermission('level', 'edit') && !hasPermission('level', 'delete') && !isSuperAdmin" 
                    ...
                    <tbody v-else>
                        <tr class=" [&>*]:p-2">
                            <td colspan="6" class="font-semibold text-center text-lg">
                                No Data!
                            </td>
                        </tr>
                    </tbody>
...
import LevelForm from '@/components/modals/levelForm.vue';
...
const handlePopOpenform = (status, item) => {
    if (status == 'add') {
        openForm.value = true;
        currentComponent.value = LevelForm;
    } else {
...
const handleConfirmDetele = (item) => {
    docData.value = item;
    elementCategory.value = "Level";
    elementName.value = item.name;
    openForm.value = true;
    currentComponent.value = ConfirmationDelete;

    if (levelData.value.length === 1 && currentPageIsLastRecord.value > 1) {

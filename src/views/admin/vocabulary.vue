<template>
    <div class="text-primary-text animate-fade-up">
        <h2 class="text-primary-dark text-lg font-bold mb-2">Vocabulary</h2>
        ...
        <div
            ...
            <div class="w-full md:w-[30%] flex items-center">
                <InputText v-model="searchQuery" type="text" placeholder="Search by word"
            ...
            <div class="flex items-center justify-end">
                <!-- <Button v-if="hasPermission('vocabulary', 'add') || isSuperAdmin"  -->
                <Button 
                ...
                <table class="min-w-full bg-primary-bg text-center" :class="{ 'opacity-50': isLoading }">
                    ...
                    <thead class="bg-primary-light text-primary-dark text-xs uppercase whitespace-nowrap">
                        <tr>
                            ...
                            <th class="px-3 sm:px-4 py-3 border border-primary-border">Word</th>
                            <th class="px-3 sm:px-4 py-3 border border-primary-border">Translation</th>
                            <th class="px-3 sm:px-4 py-3 border border-primary-border">Pronunciation</th>
                    ...
                    <!-- Table Body -->
                    <tbody class="text-primary-text text-xs" v-if="vocabularyData.length > 0">
                        <tr class="hover:bg-primary-hover whitespace-nowrap" v-for="(item, index) in vocabularyData"
                            :key="index" :class="index % 2 === 0 ? '' : 'bg-primary-30'">
                            <td class="px-3 sm:px-4 py-3 border border-primary-border">{{ index + 1 }}</td>
                            <td class="px-3 sm:px-4 py-3 border border-primary-border font-semibold">{{ item.word }}</td>
                            <td class="px-3 sm:px-4 py-3 border border-primary-border">{{ item.translation || 'N/A' }}</td>
                            <td class="px-3 sm:px-4 py-3 border border-primary-border text-xs italic">{{ item.pronunciation || 'N/A' }}</td>
                            ...
                            <td class="px-3 sm:px-4 py-3 border border-primary-border">
                                <div class="flex justify-center gap-3">
                                    <!-- <i v-if="hasPermission('vocabulary', 'edit') || isSuperAdmin"  -->
                                    <i 
                                    ...
                                    <!-- <i v-if="hasPermission('vocabulary', 'delete') || isSuperAdmin"  -->
                                    <i 
                                    ...
                                    <!-- <span v-if="!hasPermission('vocabulary', 'edit') && !hasPermission('vocabulary', 'delete') && !isSuperAdmin" 
...
import VocabularyForm from '@/components/modals/vocabularyForm.vue';
...
const handlePopOpenform = (status, item) => {
    if (status == 'add') {
        openForm.value = true;
        currentComponent.value = VocabularyForm;
    } else {
...
const handleConfirmDetele = (item) => {
    docData.value = item;
    elementCategory.value = "Vocabulary";
    elementName.value = item.word;
    openForm.value = true;
    currentComponent.value = ConfirmationDelete;

    if (vocabularyData.value.length === 1 && currentPageIsLastRecord.value > 1) {

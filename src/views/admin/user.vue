<template>
    <div class="text-primary-text animate-fade-up">
        <h2 class="text-primary-dark text-lg font-bold mb-2">User</h2>
        ...
        <div
            ...
            <div class="flex items-center justify-end">
                <!-- <Button v-if="hasPermission('user', 'add') || isSuperAdmin"  -->
                <Button 
                ...
                <table class="min-w-full bg-primary-bg text-center" :class="{ 'opacity-50': isLoading }">
                    ...
                    <thead class="bg-primary-light text-primary-dark text-xs uppercase whitespace-nowrap">
                        <tr>
                            ...
                            <th class="px-3 sm:px-4 py-3 border border-primary-border">Display Name</th>
                            <th class="px-3 sm:px-4 py-3 border border-primary-border">Username</th>
                            <th class="px-3 sm:px-4 py-3 border border-primary-border">Email</th>
                            <th class="px-3 sm:px-4 py-3 border border-primary-border">Main Role</th>
                            <th class="px-3 sm:px-4 py-3 border border-primary-border">Role</th>
                    ...
                    <!-- Table Body -->
                    <tbody class="text-primary-text text-xs" v-if="userData.length > 0">
                        <tr class="hover:bg-primary-hover whitespace-nowrap" v-for="(item, index) in userData"
                            :key="index" :class="index % 2 === 0 ? '' : 'bg-primary-30'">
                            <td class="px-3 sm:px-4 py-3 border border-primary-border">{{ index + 1 }}</td>
                            <td class="px-3 sm:px-4 py-3 border border-primary-border font-semibold">{{ item.displayName || 'N/A' }}</td>
                            <td class="px-3 sm:px-4 py-3 border border-primary-border">{{ item.username || 'N/A' }}</td>
                            <td class="px-3 sm:px-4 py-3 border border-primary-border">{{ item.email }}</td>
                            ...
                            <td class="px-3 sm:px-4 py-3 border border-primary-border">
                                <div class="flex justify-center gap-3">
                                    <!-- <i v-if="hasPermission('user', 'edit') || isSuperAdmin"  -->
                                    <i 
                                    ...
                                    <!-- <i v-if="hasPermission('user', 'delete') || isSuperAdmin"  -->
                                    <i 
                                    ...
                                    <!-- <span v-if="!hasPermission('user', 'edit') && !hasPermission('user', 'delete') && !isSuperAdmin" 
                    ...
                    <tbody v-else>
                        <tr class=" [&>*]:p-2">
                            <td colspan="10" class="font-semibold text-center text-lg">
                                No Data!
                            </td>
                        </tr>
                    </tbody>
...
const fetchRoleById = async (roleId) => {
    ...
    try {
        ...
        const response = await axios.get(`${apiURL}/lms/api/getAllDocs/Role`, { params });
        const role = response.data.data[0] || null;
        
        if (role) {
            // Cache the role data
            roleCache.value[roleId] = role;
        }
...
const getRoleName = (roleId) => {
    ...
    const role = roleCache.value[roleId];
    if (role) {
...
const handleListenToPagination = async (items) => {
    ...
    // Fetch user data for all createdBy IDs and role data for all roleId
    if (items && items.length > 0) {
        const userIds = [...new Set(items.map(item => item.createdBy).filter(Boolean))];
        const roleIds = [...new Set(items.map(item => item.roleId).filter(Boolean))];
        await Promise.all([
            ...userIds.map(userId => fetchUserById(userId)),
            ...roleIds.map(roleId => fetchRoleById(roleId))
        ]);
    }
};
...
const handleConfirmDetele = (item) => {
    docData.value = item;
    elementCategory.value = "User";
    elementName.value = item.displayName || item.username || item.email;
    openForm.value = true;
    currentComponent.value = ConfirmationDelete;

    if (userData.value.length === 1 && currentPageIsLastRecord.value > 1) {
